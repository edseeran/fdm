...
function format3(message, ...args) {
  ...
  return message.replace(RE_ARGS3, (match, identifier) => {
    return args.hasOwnProperty(identifier) ? args[identifier] : "";
  });
}
...
function createCompileWarn(code3, loc, ...args) {
  const msg = format3(warnMessages[code3] || "", ...args || []);
...
function createCompileError(code3, loc, options = {}) {
  ...
  const msg = format3((messages || errorMessages)[code3] || "", ...args || []);
...
function getWarnMessage(code3, ...args) {
  return format2(warnMessages2[code3], ...args);
}
var code = CompileErrorCodes.__EXTEND_POINT__;
var inc = incrementer2(code);
var CoreErrorCodes = {
...
function checkHtmlMessage(source, warnHtmlMessage) {
  if (warnHtmlMessage && detectHtmlTag(source)) {
    warn2(format2(WARN_MESSAGE, { source }));
  }
}
...
function compile(message, context) {
  ...
  if (__INTLIFY_JIT_COMPILATION__ && !__INTLIFY_DROP_MESSAGE_COMPILER__ && isString2(message)) {
    ...
    if (cacheKey) {
      ...
      return compileCache[cacheKey] = format4(message);
    } else {
...
function translate(context, ...args) {
  ...
  let format5 = formatScope;
  let cacheBaseKey = key;
  if (!resolvedMessage && !(isString2(format5) || isMessageAST(format5) || isMessageFunction(format5))) {
    if (enableDefaultMsg) {
      format5 = defaultMsgOrKey;
      cacheBaseKey = format5;
    }
  }
  if (!resolvedMessage && (!(isString2(format5) || isMessageAST(format5) || isMessageFunction(format5)) || !isString2(targetLocale))) {
    return unresolving ? NOT_REOSLVED : key;
  }
  if (isString2(format5) && context.messageCompiler == null) {
  ...
  const msg = !isMessageFunction(format5) ? compileMessageFormat(context, key, targetLocale, format5, cacheBaseKey, onError) : format5;
  if (occurred) {
    return format5;
  }
  ...
  if (true) {
    const payloads = {
      timestamp: Date.now(),
      key: isString2(key) ? key : isMessageFunction(format5) ? format5.key : "",
      locale: targetLocale || (isMessageFunction(format5) ? format5.locale : ""),
      format: isString2(format5) ? format5 : isMessageFunction(format5) ? format5.source : "",
      message: ret
    };
...
function resolveMessageFormat(context, key, locale, fallbackLocale, fallbackWarn, missingWarn) {
  ...
  for (let i = 0; i < locales.length; i++) {
    ...
    if ((format5 = resolveValue2(message, key)) === null) {
      format5 = message[key];
    }
    if (inBrowser2) {
      const end = window.performance.now();
      const emitter = context.__v_emitter;
      if (emitter && start && format5) {
    ...
    if (isString2(format5) || isMessageAST(format5) || isMessageFunction(format5)) {
      break;
    }
  ...
  return [format5, targetLocale, message];
}
function compileMessageFormat(context, key, targetLocale, format5, cacheBaseKey, onError) {
  const { messageCompiler, warnHtmlMessage } = context;
  if (isMessageFunction(format5)) {
    const msg2 = format5;
  ...
  const msg = messageCompiler(format5, getCompileContext(context, targetLocale, cacheBaseKey, format5, warnHtmlMessage, onError));
  if (inBrowser2) {
    const end = window.performance.now();
    const emitter = context.__v_emitter;
    if (emitter && start) {
...
function datetime(context, ...args) {
  ...
  for (let i = 0; i < locales.length; i++) {
    ...
    format5 = datetimeFormat[key];
    if (isPlainObject2(format5))
      break;
    handleMissing(context, key, targetLocale, missingWarn, type);
    from = to;
  }
  if (!isPlainObject2(format5) || !isString2(targetLocale)) {
    return unresolving ? NOT_REOSLVED : key;
  }
  ...
  if (!formatter) {
    formatter = new Intl.DateTimeFormat(targetLocale, assign2({}, format5, overrides));
...
function clearDateTimeFormat(ctx, locale, format5) {
  const context = ctx;
  for (const key in format5) {
...
function number(context, ...args) {
  ...
  if (!formatter) {
    formatter = new Intl.NumberFormat(targetLocale, assign2({}, format5, overrides));
...
function clearNumberFormat(ctx, locale, format5) {
  const context = ctx;
  for (const key in format5) {
    const id = `${locale}__${key}`;
...
function createComposer(options = {}, VueI18nLegacy) {
  ...
  function setDateTimeFormat(locale2, format5) {
    _datetimeFormats.value[locale2] = format5;
    _context.datetimeFormats = _datetimeFormats.value;
    clearDateTimeFormat(_context, locale2, format5);
  }
  function mergeDateTimeFormat(locale2, format5) {
    _datetimeFormats.value[locale2] = assign(_datetimeFormats.value[locale2] || {}, format5);
    _context.datetimeFormats = _datetimeFormats.value;
    clearDateTimeFormat(_context, locale2, format5);
  }
  ...
  function setNumberFormat(locale2, format5) {
    _numberFormats.value[locale2] = format5;
    _context.numberFormats = _numberFormats.value;
    clearNumberFormat(_context, locale2, format5);
  }
  function mergeNumberFormat(locale2, format5) {
    _numberFormats.value[locale2] = assign(_numberFormats.value[locale2] || {}, format5);
    _context.numberFormats = _numberFormats.value;
    clearNumberFormat(_context, locale2, format5);
  }
  composerID++;
...
function createVueI18n(options = {}, VueI18nLegacy) {
  {
    ...
    const vueI18n = {
      ...
      // setDateTimeFormat
      setDateTimeFormat(locale, format5) {
        composer.setDateTimeFormat(locale, format5);
      },
      // mergeDateTimeFormat
      mergeDateTimeFormat(locale, format5) {
        composer.mergeDateTimeFormat(locale, format5);
      },
      // n
      n(...args) {
      ...
      // setNumberFormat
      setNumberFormat(locale, format5) {
        composer.setNumberFormat(locale, format5);
      },
      // mergeNumberFormat
      mergeNumberFormat(locale, format5) {
        composer.mergeNumberFormat(locale, format5);
      },
      // getChoiceIndex
...
function useI18nForLegacy(instance, scope, root, options = {}) {
  ...
  function setDateTimeFormat(locale2, format5) {
    if (_composer.value) {
      _composer.value.setDateTimeFormat(locale2, format5);
      _datetimeFormats.value[locale2] = format5;
    }
  }
  function mergeDateTimeFormat(locale2, format5) {
    if (_composer.value) {
      _composer.value.mergeDateTimeFormat(locale2, format5);
    }
  }
  function getNumberFormat(locale2) {
  ...
  function setNumberFormat(locale2, format5) {
    if (_composer.value) {
      _composer.value.setNumberFormat(locale2, format5);
      _numberFormats.value[locale2] = format5;
    }
  }
  function mergeNumberFormat(locale2, format5) {
    if (_composer.value) {
      _composer.value.mergeNumberFormat(locale2, format5);
    }
  }
  const wrapper = {
    get id() {
